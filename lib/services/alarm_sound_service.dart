...
class AlarmSoundService {
  ...
  Future<void> playAlarmSound(Alarm alarm, {bool loop = true}) async {
    ...
    try {
      ...
      if (assetRingtones.containsKey(soundName)) {
        ...
        debugPrint('AlarmSoundService: Playing asset ringtone: $soundName');
      } else if (_isYoutubeOrUrl(soundName)) {
        // YouTube/URL-based ringtone - play from URL
        await _playFromUrl(player, soundName);
        debugPrint('AlarmSoundService: Playing URL sound: $soundName');
      } else if (_isLocalFile(soundName)) {
        // Local file on device
        await player.play(DeviceFileSource(soundName));
        debugPrint('AlarmSoundService: Playing local file: $soundName');
      } else {
        // In-app ringtone -- use tone generator
        await _playInAppRingtone(player, soundName, loop: loop);
        debugPrint('AlarmSoundService: Playing in-app ringtone: $soundName');
      }
      ...
      debugPrint('AlarmSoundService: Error playing sound "$soundName": $e');
  ...
  Future<void> _playFromUrl(AudioPlayer player, String url) async {
    // For YouTube URLs, we extract the audio URL
    // Note: Direct YouTube streaming requires a backend service.
    // For now, we try to play it as a direct URL source.
    // If you have a YouTube-to-audio proxy, replace this logic.

    String audioUrl = url;

    // If it's a youtube URL, we can't directly stream it
    // We store a flag and try our best
    if (url.contains('youtube.com') || url.contains('youtu.be')) {
      // Try using an invidious instance or similar proxy for audio
      final videoId = _extractYoutubeId(url);
      if (videoId != null) {
        // Use a public invidious API for audio stream
        audioUrl = 'https://inv.nadeko.net/latest_version?id=$videoId&itag=140';
      }
    }
